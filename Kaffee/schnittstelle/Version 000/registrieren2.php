...
//Die Variablen sollten nur Ihre Wertzuweisen bekommen wenn die App verwendet wird.
	$Username =  $_GET["Usernameapp"]; 
	$Password =  $_GET["Passwordapp"]; 
	$Password2 = $_GET["Password2app"]; 
...
// Create connection
	...
	if ($conn->connect_error) {
		...
		$strSQL = "SELECT name, guthaben FROM `". $lv_Tab ."` where `name` = '".ucfirst($Username) . "' and `Passwort` = '" . $Password . "'";
		//echo "$strSQL<hr>";
