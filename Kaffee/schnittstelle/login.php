...
//  Auswertung POST Input-Parameter *****************	
	...
	if (! isset($_POST["St"] )) {$lv_status    = '';} else {$lv_status    = $_POST["St"]; };
...
//*** UN und PW früfen - Wenn leer ist Rückgabe = false
	...
	// *** Auf DB Verbinden und prüfen ob username und password vorhanden sind
		...
		if ($db_conn->connect_error) {
			...
			$strSQL = "SELECT Name,Passwort,Guthaben FROM `". $db_tab_kaffee ."` where `name` = '".ucfirst($lv_username) . "' and `Passwort` = '" . $lv_password . "'";
			...
			if($db_result->num_rows <> 0){
				...
				$la_Status["GHn"] =  $lv_row[2];
				
				//echo "Kann jetzt abbuchen";
			} else {
				$la_Status["S"] = false;
			} //$db_result ende
